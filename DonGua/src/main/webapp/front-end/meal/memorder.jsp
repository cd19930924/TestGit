...
<%
	...
	List<MealOrderVO> list = mealOrderSvc.findByMemId(id);
	pageContext.setAttribute("list", list);
	String detailId = request.getParameter("detail");
	pageContext.setAttribute("detailId", detailId);
%>
            ...
            <div class="row w-100">
                <div class="col-lg-12 col-md-12 col-12">
                    ...
                    <ul class="nav nav-tabs">
                        <li class="nav-item">
                            <a class="nav-link active" aria-current="page" href='<%=request.getContextPath()%>/front-end/meal/memorder.jsp'>歷史訂單</a>
                        </li>
                        <li class="nav-item">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" href='<%=request.getContextPath()%>/front-end/meal/memorderclose.jsp'>已結單</a>
                        </li>
                         <li class="nav-item">
...
<c:forEach var="mealOrderVO" items="${list}">
									<tr>
										...
										<td>
											...
											<button type="button" class="show" style="border-radius:5px" id="show${mealOrderVO.mealOrderId}">
												查看</button> <!-- Modal -->
											<div
												class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable detail" 
												id="detail${mealOrderVO.mealOrderId}" >
												<div class="modal-content">
													...
													<div class="modal-footer">
														<button type="button" class="btn btn-primary hide" id="hide${mealOrderVO.mealOrderId}">確定</button> 
													</div>
												</div>
											</div>
										</td>
										...
										<c:if test="${mealOrderVO.orderStatus == 0}">
												<form action="order.do" method="post">
													<input type="hidden" name="mealOrderId" value="${mealOrderVO.mealOrderId}"> 
														<input type="hidden" name="action" value="cancel_order_onall">
 ...
 <c:forEach var="mealOrderVO" items="${list}">
      $("#show${mealOrderVO.mealOrderId}").click(function(){
        $("#detail${mealOrderVO.mealOrderId}").show();
      });
      $("#hide${mealOrderVO.mealOrderId}").click(function(){
        $("#detail${mealOrderVO.mealOrderId}").hide();
      });
 </c:forEach>
    });
    </script> 
    ...
    <script>
 <c:forEach var="mealOrderVO" items="${list}">
 	if( ${detailId} == ${mealOrderVO.mealOrderId} ) {
	 	$("#detail${mealOrderVO.mealOrderId}").show()
	 }
 </c:forEach>
